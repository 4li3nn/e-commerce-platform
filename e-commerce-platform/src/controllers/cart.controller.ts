...
export class CartController {
  constructor(
    ...
    @inject('authentication.userRepository') private user: UserProfile,
  ...
  @authenticate('jwt')
  @get('/carts-check')
  async yourAuthenticatedEndpoint(): Promise<string> {
    // Bạn có thể truy cập thông tin user từ biến user
    const userId = this.user[securityId]; // Lấy ID của user từ UserProfile

    // Hoặc có thể lấy thông tin user từ database (nếu cần)
    const user = await this.userRepository.findById(userId);

    // Trả về thông tin user hoặc thực hiện các hành động khác
